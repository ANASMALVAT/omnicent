...
const SingleMessage = ({message,isNextMessageSamePerson} : MessageProps) => {
    return (
        <div className={cn('flex items-end',{'justify-end': message.isUserMessage})}>
            <div className={cn('relative flex h-6 w-6 aspect-square items-center justify-center',{
                ...
                "order-1 bg-zinc-950 rounded-sm":!message.isUserMessage,
                invisible: isNextMessageSamePerson
            })}>
                {message.isUserMessage ?
                (
                    <Icons.user className="fill-zinc-200 text-zinc h-3/4 w-3/4"/>

                ): 
                (
                    <Icons.logo className="fill-zinc-400 h-3/4 w-3/4"/>
                )}
            </div>
